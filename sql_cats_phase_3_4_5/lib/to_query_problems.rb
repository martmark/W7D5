...
def just_like_orange
  ...
  execute(<<-SQL)
  SELECT
    cats_2.name, cats_2.breed
  FROM
    cats AS cats_1
  JOIN
    cats AS cats_2 on cats_1.breed = cats_2.breed
  WHERE 
    cats_1.name = 'Orange'
  AND
    cats_2.name != 'Orange'
...
def toys_that_jet_owns
  ...
  FROM
    cats AS cats1
  JOIN
    cattoys on cattoys.cat_id = cats1.id
  JOIN
    toys on cattoys.toy_id = toys.id
  JOIN
    cattoys as cattoys2 on toys.id = cattoys2.toy_id
  JOIN
    cats as cats2 on cats2.id = cattoys2.cat_id
  WHERE
    cats1.name = 'Jet'
  AND
    cats2.name != 'Jet'
